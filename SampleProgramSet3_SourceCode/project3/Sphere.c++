...
Sphere::Sphere(float xPos, float yPos, float zPos, float rad) {
	bColor[0] = 0.0; //red
	bColor[1] = 0.0; //green
	bColor[2] = 0.0; //blue

	defineSphere(xPos, yPos, zPos, rad);
}
...
void Sphere::defineSphere(float xPos, float yPos, float zPos, float rad) {
	vec3* coordArr = new vec3[SphTotalPts];
	vec3* normalArr = new vec3[SphTotalPts];

	double th = 0.0;
	double deltaTh = 2.0 * M_PI/(SphNumHorizontalPts - 1);
	double phiVal = M_PI;
	double deltaPhi = M_PI / SphNumVerticalPts;

	for (int currVertPt = 0; currVertPt <= SphNumVerticalPts; currVertPt++) {
		...
		for (int currHorizPt = 0; currHorizPt <= SphNumHorizontalPts; currHorizPt++) {
			...
			int currIndex = currVertPt * 2 * SphNumHorizontalPts + horizBufferPt;
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vec3) * SphTotalPts, coordArr, GL_STATIC_DRAW);
	glVertexAttribPointer(pvaLoc_mcPosition, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(pvaLoc_mcPosition);

	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vec3) * SphTotalPts, normalArr, GL_STATIC_DRAW);
...
void Sphere::renderSphere() {
	...
	glBindVertexArray(vao);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, SphTotalPts - 1);
}
