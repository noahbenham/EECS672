...
uniform vec4 p_ecLightSource = vec4(0.7, 0.2, 1.0, 0.0); // externalize later
...
{
	float factor = dot(normalize(p_ecLightSource.xyz), ec_nHat);
	if (factor < 0) // light and viewer on opposite sides
		factor = 0;

	// Use for loop for multiple light sources
